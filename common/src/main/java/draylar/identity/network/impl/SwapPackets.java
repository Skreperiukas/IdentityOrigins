...
import draylar.identity.sound.ModSounds;
        ...
        NetworkManager.registerReceiver(NetworkManager.Side.C2S, NetworkHandler.IDENTITY_REQUEST, (buf, context) -> {
            ...
            if(validType) {
                ...
                context.getPlayer().getServer().execute(() -> {
                    ...
                    if((IdentityConfig.getInstance().enableClientSwapMenu() || context.getPlayer().hasPermissionLevel(3))
                            && PowerHolderComponent.getPowers(context.getPlayer(), ChangeIdentityPower.class).stream().anyMatch(Power::isActive)) {
                        ...
                        ModSounds.playSoundForPlayerAndNearbyPlayers(context.getPlayer().getWorld(), context.getPlayer().getBlockPos(), ModSounds.CHANGE_IDENTITY, SoundCategory.PLAYERS, 1f, 1f, IdentityConfig.getInstance().soundDistance());
